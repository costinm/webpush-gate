...
//
// Held in LinkLocalRegistry, updated periodically or on CON/AP events.
type DirectActiveInterface struct {
	mesh.ActiveInterface
...
// If we have a p2p interface, attempt to connect to the AP. Will work if this device isn't an AP itself,
...
func (gw *LLDiscovery) ensureConnectedUp(laddr *net.UDPAddr, node *mesh.DMNode) error {
	...
	for {
		hasUp := false
		for _, a := range gw.DirectActiveInterfaces {
...
func (gw *LLDiscovery) isMeshClient() bool {
	for _, n := range gw.DirectActiveInterfaces {
...
func ips(interfaces map[string]*DirectActiveInterface) []*net.UDPAddr {
	res := []*net.UDPAddr{}
	for _, a := range interfaces {
		...
		if a.IP4 != nil && !mesh.IsRFC1918(a.IP4) {
...
// 64 byte public key
// 64 byte signature
func mcMessage(gw *LLDiscovery, i *DirectActiveInterface, isAck bool) []byte {
...
func unicastReaderThread(gw *LLDiscovery, c net.PacketConn, iface *DirectActiveInterface) {
	iname := ""
	if iface != nil {
...
// Multicast registration server. One for each interface
func (gw *LLDiscovery) multicastReaderThread(c net.PacketConn, iface *DirectActiveInterface) {
	defer func() {
		c.Close()
	...
	for {
		...
		gw.OnLocalNetworkFunc(directNode, addr,
			strings.Contains(addr.Zone, "p2p"))
...
//func (lm *LinkLocalRegistry) getIf(zone string) *DirectActiveInterface {
//	lm.activeMutex.Lock()
//	defer lm.activeMutex.Unlock()
//	a, found := lm.DirectActiveInterfaces[zone]
//	if !found {
//		log.Println("Node no longer DirectActiveInterface ", zone)
//		return nil
//	}
//	return a
//}

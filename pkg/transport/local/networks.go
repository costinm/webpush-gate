...
// RefreshNetworks will update the list of DirectActiveInterface networks, and ensure each has a listener.
...
func (gw *LLDiscovery) RefreshNetworks() {
	...
	unchanged := map[string]*DirectActiveInterface{}
	...
	for nname, existing := range gw.DirectActiveInterfaces {
		...
		changed = true
		// No longer DirectActiveInterface. The socket is probably closed already
	...
	names := []string{}
	// Already DirectActiveInterface - copy over the socket
	for _, a := range newAct {
		...
		if true || hasAp {
			...
			if a.multicastRegisterConn != nil {
				continue // was DirectActiveInterface before
			}
	...
	gw.DirectActiveInterfaces = newAct
	t1 := time.Now()
...
func ActiveNetworks(gw *LLDiscovery) (map[string]*DirectActiveInterface, error) {
	...
	anets := map[string]*DirectActiveInterface{}
	...
	for _, ifi := range ifaces {
		...
		a := &DirectActiveInterface{
			ActiveInterface: mesh.ActiveInterface{
				Name:  ifi.Name,
				IP6LL: ip,
				IP4:   ip4,
				IPPub: ippub,
...
func findActive(tofind *DirectActiveInterface, in map[string]*DirectActiveInterface) *DirectActiveInterface {

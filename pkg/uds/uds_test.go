...
func TestUDS(t *testing.T) {
	fn := "testUDS"
	us, err := NewServer(fn, mux.DefaultMux)
	defer os.Remove(fn)
	if err != nil {
	...
	go us.Start()
	mux.DefaultMux.AddHandler("srv", mux.HandlerCallbackFunc(func(cmdS string, meta map[string]string, data []byte, replyTo *mux.MsgConnection) {
		log.Println(cmdS, meta, data)
	}))

	cc, err := Dial(fn, mux.DefaultMux, map[string]string{})
	if err != nil {
		t.Error("Failed to Redial", err)
		return
	}
	go cc.HandleStream()
	mux.DefaultMux.AddHandler("client", mux.HandlerCallbackFunc(func(cmdS string, meta map[string]string, data []byte, replyTo *mux.MsgConnection) {
		log.Println(cmdS, meta, data)
	}))

...
func (tp *TcpProxy) Proxy() error {
	...
	go tp.proxyClientToServer(tp.ServerOut, tp.ClientIn, errCh)
...
func (tp *TcpProxy) proxyServerToClient(errch chan error) (int64, error) {
	...
	if tp.ServerIn == nil || tp.ClientOut == nil {
		err = io.EOF
		log.Println("NULL ", tp.ServerIn, tp.ClientOut)
	} else {
		if n, err = tp.CopyBuffered(tp.ClientOut, tp.ServerIn, true); err != nil {
	...
	closeRead(tp.ServerIn) // likely already closed (remoteIn.Read returned error or EOF)
...
func (tp *TcpProxy) proxyClientToServer(remoteOut io.Writer, localIn io.ReadCloser, errch chan error) {
	...
	if tp.Initial != nil {
		_, err = tp.ServerOut.Write(tp.Initial)
	}

	// TODO: err, n to be sent on a channel, for metrics
	if err != nil {

	} else if tp.ClientIn == nil || tp.ServerOut == nil {
		err = io.EOF
		log.Println("NULL ", localIn, tp.ServerOut)
	} else {
		if _, err = tp.CopyBuffered(tp.ServerOut, localIn, false); err != nil {
			if err1, ok := err.(*net.OpError); ok && err1.Err == syscall.EPIPE {
				// typical close
				err = io.EOF
			}

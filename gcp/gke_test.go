...
//   services_secondary_range_name:"gke-big1-services-6c27336c"
...
// legacy_abac:{}
// network_policy:{provider:CALICO  enabled:true}
...
func GetClusters(p string) ([]string, error) {
	c := []string{}
	...
	for _, cc := range resp.Clusters {
		c = append(c, cc.Name)
		log.Println(cc.Name, cc.Location, cc.Endpoint, cc.MasterAuth.ClusterCaCertificate)
	}
...
func TestGKE(t *testing.T) {
	...
	kc, err := InitK8S("costin-asm1", "us-central1-c", "cloudrun")
	if err != nil {
		t.Fatal(err)
	}
	pl, err := kc.CoreV1().Pods("").List(ctx, metav1.ListOptions{})

...
var (
	...
	// Has to be separate because framing is trickier
	serverPortH2 = server.String("portH2", ":5228", "Server to listen for long lived connections using standard protocol")

	pushService = ua.String("server", "http://localhost:5222", "Base URL for the push service")
...
func showCurl() {
	...
	vapid := auth.NewVapid(pub64, priv64)
	msg, err := ioutil.ReadAll(os.Stdin)
	if err != nil {
		fmt.Println("Failed to read message")
		os.Exit(3)
	}
	to, err := auth.SubscriptionFromJSON([]byte(os.Getenv(Subscription)))
...
func sendMessage() {
	...
	req, err := send2.NewRequest(to, string(msg), 0, vapid)
